...
class InheritanceGraph(object):
    ...
    def __init__(self, class_names, show_builtins=False):
        ...
        *class_names* is a list of child classes to show bases from.
        ...
        if len(self.all_classes) == 0:
            raise ValueError("No classes found for inheritance diagram in %s", class_names)
    ...
    def _import_class_or_module(self, name):
        ...
        # add those 2 lines to prevent wierd failures TC,feb.2009
        if not path:
           path = fullname
        ...
        try:
            ...
            raise ValueError(
                "Could not import class or module '%s' specified for inheritance diagram. Try to run the module manually to check whether it runs without errors." % name)
...
def html_output_graph(self, node):
    ...
    if os.path.exists('.static'):
        png_path = os.path.join('.static', name + ".png")
        path = '.static'
    ...
    path = path.replace('.static', 'html/_static')
    # hack for openalea
...
def latex_output_graph(self, node):
    ...
    name = "inheritance%s" % graph_hash 
    if os.path.exists('.static'):
        pdf_path = os.path.join('.static', name + ".pdf")
        graph.run_dot(['-Tpdf', '-o%s' % pdf_path],
                  name, parts, graph_options={'size': '"6.0,6.0"'})
        return '\\includegraphics{../../%s}' % pdf_path
        print('111111111111111111111')
    elif os.path.exists('./doc/.static'):
        pdf_path = os.path.join('./doc/.static', name + ".pdf")
        graph.run_dot(['-Tpdf', '-o%s' % pdf_path],
                  name, parts, graph_options={'size': '"6.0,6.0"'})
        return '\\includegraphics{../../%s}' % pdf_path
